...
export const weeklyStats = async (_req, res) => {
  ...
  res.json({ series, done, total, range: { start: startOfWeek.toISOString(), end: endOfWeek.toISOString() } });
};
...
export const monthlyStats = async (_req, res) => {
  ...
  res.json({ series, done, total, range: { start: startOfMonth.toISOString(), end: endOfMonth.toISOString() } });
};
// server/src/controllers/habit.controller.js
